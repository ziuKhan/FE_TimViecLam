    ...
    <div class="w-11/12 lg:w-11/12 mx-auto py-8 lg:flex ">
        <div class="w-full p-2 lg:p-3">  
            <div class="flex justify-between mb-4">
                <h1 class="text-xl lg:text-3xl font-bold">{{ store.paginateJobs?.total }} việc làm <span class="text-red-600">{{ store.paginateJobs?.keyword }}</span> tại Việt Nam</h1>
                <button @click="showModal" class="border w-24 lg:w-32 h-8 lg:h-10 border-red-600 text-red-600 px-1 bg-white rounded-md">Bộ lọc</button>
                <a-modal v-model:open="open" width="800px" title="Bộ lọc" @ok="handleOk" okText="Xác nhận" cancelText="Huỷ bỏ" :maskClosable=false 
            ...
            <div  class="w-full flex justify-center mt-5">
                <a-pagination  v-model:current="store.current" :total="store.paginateJobs?.total" :page-size="store.paginateJobs?.pageSize" show-less-items />
            </div>
