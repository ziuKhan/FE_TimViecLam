<template>
  <div
    class="success-container min-h-screen flex flex-col items-center justify-center px-4 py-16 bg-gradient-to-b from-blue-50 to-white"
  >
    <div
      class="success-card bg-white shadow-lg rounded-2xl p-8 w-full max-w-lg text-center relative overflow-hidden"
    >
      ...
      <div class="success-icon relative mb-6 mx-auto">
        <div
          class="animate-success-ring w-24 h-24 rounded-full border-4 border-red-500 flex items-center justify-center"
        >
      ...
      <div class="transaction-details bg-gray-50 p-4 rounded-lg mb-8 relative">
        <div class="flex justify-between mb-2">
          ...
          <span class="font-semibold text-gray-800">{{
            route.query?.orderCode || 'Không có mã giao dịch'
          }}</span>
        </div>
      ...
      <div class="countdown-container relative mb-6">
        <div
          class="countdown-circle mx-auto w-16 h-16 rounded-full flex items-center justify-center border-4 border-red-500"
        >
          <span class="text-xl font-bold text-red-600">{{ countdown }}</span>
        </div>
        <div class="w-full bg-gray-200 rounded-full h-3 mt-3">
          <div
            class="bg-red-600 h-3 rounded-full transition-all duration-1000"
      ...
      <button
        @click="goToHome"
        class="relative bg-red-600 hover:bg-red-700 text-white py-3 px-8 rounded-lg font-medium transition duration-300"
      >
        Về trang chủ ngay
      </button>
      ...
      <!-- Huy hiệu VIP -->
      <div
        class="absolute top-4 right-4 bg-amber-100 text-amber-700 px-3 py-1 rounded-full font-semibold flex items-center"
      >
    ...
    <div class="mt-8 text-center max-w-md relative">
      <p class="text-gray-600">
        ...
        <a href="#" class="text-blue-600 hover:underline">liên hệ đội hỗ trợ</a> của chúng tôi.
      </p>
    </div>
  </div>
</template>
