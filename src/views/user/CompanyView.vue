...
const getData = async () => {
  ...
  try {
    const [companies, jobs] = await Promise.all([
      apiService.get('companies/client/' + id),
      apiService.get('jobs/client' + params)
    ])
  ...
  <div v-else class="theme_gray_no_border">
    <header class="theme_blackred w-full min-h-56 py-8">
      <div class="w-11/12 mx-auto flex gap-x-6">
        <div
          class="min-w-32 max-h-32 max-w-32 lg:min-w-40 lg:max-w-40 lg:max-h-40 bg-white rounded-md p-2"
        >
          <img
            loading="lazy"
            class="h-full object-contain"
            :src="linkUploads('company/' + dataCompany.logo)"
            alt="#"
          />
        </div>
        <div class="text-white w-7/12">
          ...
          <div class="text-sm font-normal flex gap-y-3 lg:gap-0 lg:flex-nowrap flex-wrap">
            <div class="mr-5 flex gap-x-2 w-full lg:w-auto">
              ...
              {{
                Array.isArray(dataCompany.address)
                  ? dataCompany.address.length > 0
                    ? dataCompany.address.join(', ')
                    : ''
                  : dataCompany.address
              }}
            </div>
    ...
    <div class="w-11/12 mx-auto py-10 flex flex-wrap lg:flex-nowrap">
      <div class="w-full lg:w-4/6 lg:pr-5">
        <div class="bg-white shadow-gray-500 drop-shadow-lg rounded-xl p-5">
          ...
          <div
            v-html="dataCompany.description"
            class="font-normal text-base font-sans leading-9 overflow-hidden"
          ></div>
        </div>
      </div>
