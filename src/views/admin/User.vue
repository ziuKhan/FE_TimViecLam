...
const openModal = ref<boolean>(false)
  ...
  const refreshInput = () => {
    Object.assign(form, {
      _id: '', name: '', email: '', password: '', gender: '', address: '', role: '', isActive: false,
      company: { _id: '', name: '' }
    })
  }
  const handleRole = () => {
...
const handleCompanyChange = (companyId: string) => {
    if (form.company) {
        ...
        const selectedCompany = dataCompanies.value.find((company: ICompany) => company._id === companyId);
        if (selectedCompany) {
...
<template>
            ...
            <div class="flex justify-between">
                ...
                <div class="w-3/4 flex" v-permission="'GET /api/v1/users'">
                    <a-input-search placeholder="Vui lòng nhập thông tin cần tìm kiếm" enter-button="Tìm kiếm"
                        v-model:value="keySearch" @search="getData" class="w-3/6" />
                    <a-select v-model:value="valueFilter.role" class="w-1/5 ml-5" placeholder="Chọn role"
                        @change="handleRole">
                ...
                <button class="bg-[#21aa55] hover:bg-green-500 text-white rounded-[7px] px-5 font-medium "
                    @click="handleOpenModal()" v-permission="'POST /api/v1/users'">Thêm
                    mới</button>
            </div>

            <div class="mt-3 border rounded-[10px]">
                <a-table :columns="columns" :data-source="data" :loading="loading"
                    :pagination=dataMeta @change="handleTableChange">
                        ...
                        <template v-else-if="column.dataIndex === '_id'">
                            <div class="flex">
                                <button type="button"
                                    ...
                                    @click="getByID(text)" v-permission="'PATCH /api/v1/users/:id'">
                                    <img loading="lazy" class=" h-[24px]"
                                        src="@/assets/image/icon/icons8_update.svg" alt="">
                                </button>

                                <a-popconfirm title="Bạn có chắc muốn xoá?" ok-text="Có" cancel-text="Không"
                                    :loading="loading" @confirm="deleteByID(text)" @cancel=""
    ...
    <a-modal :title="form._id ? 'Cập nhật tài khoản' : 'Tạo mới tài khoản'"
        :okText="form._id ? 'Cập nhật' : 'Thêm mới'" :width="850" v-model:open="openModal"
