...
import { CheckCircleOutlined, CopyOutlined, DeleteOutlined, EditOutlined, SettingOutlined } from '@ant-design/icons-vue';
    ...
    <a-layout-content style="margin: 0 16px">
        ...
        <div class="p-6 bg-white min-h-[360px] rounded-[10px]">
            <div class="flex justify-between">
                <a-input-search v-permission="'GET /api/v1/customer-approval'"
                ...
                <button class="bg-[#21aa55] hover:bg-green-500 text-white rounded-[7px] px-5 font-medium "
                    @click="store.handleOpenModal()" v-permission="'POST /api/v1/customer-approval'">Thêm
                    mới</button>
            </div>
                ...
                <a-table :columns="columns" :data-source="store.data" :loading="store.loading" :pagination=store.dataMeta
                    ...
                    <template #bodyCell="{ column, text, index }">
                        ...
                        <template v-else-if="column.dataIndex === 'status'">
                            <a-tag :color="text === 'CD' ? 'blue' : text === 'CN' ? 'green' : 'red'">{{ text === 'CD' ? 'Chờ duyệt' : text === 'CN' ? 'Đã duyệt' : 'Từ chối' }}</a-tag>
                        </template>
                        <template v-else-if="column.dataIndex === 'logo'">
                            <img :src="linkUploads('company/'+text)" alt="Logo" class="w-10 h-10 rounded-[5px] object-cover">
                        </template>
                        <template v-else-if="column.dataIndex === '_id'">
                            <button type="button"
                                class="mr-2  bg-[#17b63aec] hover:bg-[#66df80ec] rounded-[5px] px-[10px] py-1 h-8 "
                                @click="store.getByID(text, true)" v-permission="'PATCH /api/v1/skills/:id'">
