<template>
    <a-drawer title="Thông tin chi tiết hồ sơ xin việc" :width="750" :open="store.openDrawer"
        :footer-style="{ textAlign: 'right' }" @close="store.openDrawer = false" :maskClosable="false">
        <div class="px-10 py-5">
            <a-row :gutter="16" class="gap-y-2 text-base pb-5">

                <a-col :span="24" class="text-base font-bold">
                    THÔNG TIN CÁ NHÂN
                </a-col>
                <a-col :span="12">
                    Họ và tên: {{ data?.name }}
                ...
                <a-col :span="24">
                    Link CV: <a class="text-blue-600 text-[16px]" :href="linkUploads('cv/' + store.form.url)"
                        target="_blank">{{
                            store.form.url }}</a>
                </a-col>
                ...
                <a-col :span="24">
                    Tên việc làm: {{ store.form.jobId.name }}
                </a-col>
                <a-col :span="12">
                    Kỹ năng:
                ...
                <a-col :span="24">
                    <a-table :columns="columns" :data-source="store.form.history" :loading="store.loading"
                        :pagination="false">
