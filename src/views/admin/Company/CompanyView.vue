...
<template>
  <a-layout-header
    ...
    >Trang quản lý công ty</a-layout-header
  >
  <a-layout-content style="margin: 0 16px">
    ...
    <div class="p-6 bg-white min-h-[360px] rounded-[10px]">
      <div class="flex justify-between">
        ...
        <button
          class="bg-[#21aa55] hover:bg-green-500 text-white rounded-[7px] px-5 font-medium h-[32px]"
          @click="store.handleOpenModal()"
          v-permission="'POST /api/v1/companies'"
        >
          Thêm mới
        </button>
      </div>
          ...
          <template #bodyCell="{ column, text, index }">
            ...
            <template v-else-if="column.dataIndex === '_id'">
              <button
                ...
                v-permission="'PATCH /api/v1/companies/:id'"
              >
