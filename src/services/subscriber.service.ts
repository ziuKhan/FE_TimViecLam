...
class Subscriber {
  ...
  getByEmailApi = async (email: any): Promise<any> => {
    try {
  ...
  createOrUpdateApi = async (data: any): Promise<any> => {
    try {
  ...
  updateApi = async (data: any): Promise<any> => {
    try {
      const response = await apiClient.patch(moduleName, data)
      return response.data
    } catch (err: any) {
      handleApiError(err)
    }
